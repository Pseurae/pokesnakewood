...
EWRAM_DATA static bool8 sInPartyMenu = FALSE;
...
EWRAM_DATA static bool8 sIsMonBeingMoved = FALSE;
EWRAM_DATA static u8 sMovingMonOrigBoxId = 0;
EWRAM_DATA static u8 sMovingMonOrigBoxPos = 0;
EWRAM_DATA static bool8 sAutoActionOn = FALSE;
EWRAM_DATA static bool8 sJustOpenedBag = FALSE;
...
static void VBlankCB_PokeStorage(void)
{
    ...
    if (sPaletteSwapBuffer && !gPaletteFade.bufferTransferDisabled && !gPaletteFade.active && !sStorage->transferWholePlttFrames) {
      ...
      RequestDma3Copy(&gPlttBufferFaded[(12+16)*16], (void*)(PLTT + 0x200 + 0x180), 32*4, 0);
    } else {
...
static void GiveChosenBagItem(void)
{
    ...
    if (itemId != ITEM_NONE)
    {
        ...
        if (sInPartyMenu)
        {
            ...
            SetMonFormPSS(&gPlayerParty[pos].box);
        }
        else
        {
...
static void CreatePartyMonsSprites(bool8 visible)
{
    ...
    LoadCompressedPalette(GetMonFrontSpritePal(&gPlayerParty[0]), (0+1)* 16 + 0x100, 32);
    ...
    for (i = 1; i < PARTY_SIZE; i++)
    {
        ...
        if (species != SPECIES_NONE)
        {
            ...
            LoadCompressedPalette(GetMonFrontSpritePal(&gPlayerParty[i]), paletteNum*16 + 0x100, 32);
...
static void SetPlacedMonSprite(u8 boxId, u8 position)
{
    ...
    if (boxId == TOTAL_BOXES_COUNT) // party mon
    {
        ...
        if (sStorage->partySprites[position]->oam.paletteNum == IndexOfSpritePaletteTag(PALTAG_DISPLAY_MON)) {
            ...
            LoadCompressedPalette(GetMonFrontSpritePal(&gPlayerParty[position]), paletteNum*16 + 0x100, 32);
...
static void TakeItemFromMon(u8 cursorArea, u8 cursorPos)
{
    ...
    else
    {
        ...
        SetMonFormPSS(&gPlayerParty[cursorPos].box);
    }
