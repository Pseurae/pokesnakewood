...
static u8 DisplaySelectionWindow(u8 windowType)
{
    ...
    for (i = 0; i < sPartyMenuInternal->numActions; i++)
    {
        ...
        AddTextPrinterParameterized4(sPartyMenuInternal->windowId[0], FONT_SHORT, cursorDimension, (i * 16) + 1, letterSpacing, 0, sFontColorTable[fontColorsId], 0, sCursorOptions[sPartyMenuInternal->actions[i]].text);
    }
...
static void PartyMenuPrintText(const u8 *text)
{
    ...
    AddTextPrinterParameterized2(6, FONT_SHORT, text, GetPlayerTextSpeedDelay(), 0, TEXT_COLOR_DARK_GRAY, TEXT_COLOR_WHITE, TEXT_COLOR_LIGHT_GRAY);
}
...
static bool8 CanUseHM(struct Pokemon *mon, u8 hm)
{
    ...
    if (MonKnowsMove(mon, sTMHMMoves[NUM_TECHNICAL_MACHINES + hm]) 
        || (CanLearnTeachableMove(GetMonData(mon, MON_DATA_SPECIES), sTMHMMoves[NUM_TECHNICAL_MACHINES + hm]) 
...
{
    InitPartyMenu(PARTY_MENU_TYPE_FIELD, PARTY_LAYOUT_SINGLE, PARTY_ACTION_GIVE_ITEM, FALSE, PARTY_MSG_GIVE_TO_WHICH_MON, Task_HandleChooseMonInput, CB2_ReturnToBagMenu);
